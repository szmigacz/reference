...
### Steps specific to the pytorch version to run and time
    ...
    SEED=1
    NOW=`date "+%F-%T"`
    sudo nvidia-docker run -it --rm --ipc=host -v $HOME/reference/rnn_translator/data:/data gnmt:latest "./run_and_time.sh" $SEED |tee benchmark-$NOW.log

### one can control which GPUs are used with the NV_GPU variable
    sudo NV_GPU=0 nvidia-docker run -it --rm --ipc=host -v $HOME/reference/rnn_translator/data:/data gnmt:latest "./run_and_time.sh" $SEED |tee benchmark-$NOW.log
